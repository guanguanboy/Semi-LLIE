...
class CMFE(nn.Module):
    def __init__(self, dim, mult=4):
        ...
        hidden_features = mult * dim
        padding_mode='reflect'
        self.pwconv = nn.Conv2d(hidden_features * 2, hidden_features * 2, 1, 1, 0, bias=bias)
        #self.dwconv = nn.Conv2d(hidden_features * 2, hidden_features * 2, 3, 1, 1, bias=bias, padding_mode=padding_mode, groups=hidden_features * 2)
        
        self.project_out_0 = nn.Conv2d(hidden_features, hidden_features*2, kernel_size=1, bias=bias)
        self.mlp = nn.Conv2d(in_channels, in_channels, 1, 1, 0, bias=True)
    ...
    def forward(self, x):
        ...
        x1, x2 = self.pwconv(x).chunk(2, dim=1)
        x = F.gelu(x1) * x2
        x = self.project_out_0(x)        

        x = self.project_out(x+residual)
...
if __name__ == '__main__':
    ...
    model = RetinexFormer(stage=1,n_feat=40,num_blocks=[1,2,2]).to(device=device)
    print(model)
