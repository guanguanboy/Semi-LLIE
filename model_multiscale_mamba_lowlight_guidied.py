...
class SS2D(nn.Module):
    def __init__(
        ...
        self.point_wise_conv = nn.Conv2d(self.d_inner*2, self.d_inner, kernel_size=1, bias=True)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.fuse = GFM(hidden_dim)


    def forward(self, input, x_size, inp_feats):
        # x [B,HW,C]
        B, L, C = input.shape
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_size, inp_feats):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                x = blk(x, x_size, inp_feats)
...
class MambaLowlight(nn.Module):
    ...
    def forward_features(self, x, illu_feats):
    ...
    def forward(self, x):
        ...
        illu_map, illu_feats = self.estimator(x)

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(torch.cat([x, illu_map], dim=1))
            x = self.conv_after_body(self.forward_features(x,illu_feats)) + x
            ...
            res = self.conv_after_body(self.forward_features(x_first, illu_feats)) + x_first
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size, inp_feats):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, inp_feats), x_size))) + x
